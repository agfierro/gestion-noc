...
NOC.Articulos=(()=>{
 ...
 async function render(search=""){
   ...
   ${f.map(r=>`<tr><td><strong>${NOC.App.esc(r.nombre_producto)}</strong><br><span class="muted">${NOC.App.esc(r.estilo||"")}</span></td><td>${NOC.App.money(r.precio_venta)}</td><td>${NOC.App.money(r.precio_coste)}</td><td>${NOC.App.money(Number(r.precio_venta||0)-Number(r.precio_coste||0))}</td><td>${NOC.App.esc(r.tipo_prenda||r.tipo_articulo||"")}</td><td>${NOC.App.esc(r.fabrica||"")}</td><td>${r.anio??""}</td><td>${NOC.App.esc(r.mes||"")}</td><td class="actions"><button class="btn btn-small" onclick="NOC.Articulos.openEditor('${r.id}')">Editar</button><button class="btn btn-small btn-danger" onclick="NOC.Articulos.del('${r.id}')">Borrar</button></td></tr>`).join("")||`<tr><td colspan="9" class="empty">No hay artículos.</td></tr>`}
   </tbody></table></div></div>`;
 ...
 function value(r,key){
   ...
   return {
     ...
     margen,
     tipo:r.tipo_prenda||r.tipo_articulo||"",
     fabrica:r.fabrica||"",
