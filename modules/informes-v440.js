...
NOC.Informes=(()=>{
  ...
  function asegurarEstiloSelectoresBuscables(){
    if(document.getElementById("nocSearchSelectStyle"))return;
    const style=document.createElement("style");
    style.id="nocSearchSelectStyle";
    style.textContent=`
      .noc-search-select{position:relative;width:100%}
      .noc-search-select-native{position:absolute!important;opacity:0!important;pointer-events:none!important;width:1px!important;height:1px!important;overflow:hidden!important}
      .noc-search-select-trigger{
        width:100%;min-height:42px;display:flex;align-items:center;justify-content:space-between;gap:10px;
        padding:9px 12px;border:1px solid #dfe1e2;border-radius:10px;background:#fff;color:#171717;
        font:inherit;font-size:13px;text-align:left;cursor:pointer;transition:border-color .16s ease,box-shadow .16s ease,background .16s ease
      }
      .noc-search-select-trigger:hover{border-color:#b9babb;background:#fff}
      .noc-search-select-trigger:focus-visible{outline:none;border-color:#111;box-shadow:0 0 0 2px rgba(0,0,0,.08)}
      .noc-search-select-label{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
      .noc-search-select-chevron{flex:0 0 auto;font-size:11px;color:#777;transition:transform .16s ease}
      .noc-search-select.open .noc-search-select-chevron{transform:rotate(180deg)}
      .noc-search-select-panel{
        position:absolute;z-index:1200;top:calc(100% + 5px);left:0;right:0;
        border:1px solid #d8d9d7;border-radius:12px;background:#fff;
        box-shadow:0 16px 38px rgba(0,0,0,.14);overflow:hidden
      }
      .noc-search-select-search-wrap{padding:9px;border-bottom:1px solid #ecece9;background:#fafaf8}
      .noc-search-select-search{
        width:100%;min-height:39px;padding:8px 10px;border:1px solid #d8d9d7;border-radius:8px;
        background:#fff;color:#111;font:inherit;font-size:13px
      }
      .noc-search-select-search:focus{outline:none;border-color:#111;box-shadow:0 0 0 2px rgba(0,0,0,.06)}
      .noc-search-select-options{max-height:250px;overflow:auto;padding:5px}
      .noc-search-select-option{
        width:100%;display:block;padding:9px 10px;border:0;border-radius:8px;background:transparent;
        color:#1b1b1b;font:inherit;font-size:13px;text-align:left;cursor:pointer
      }
      .noc-search-select-option:hover,.noc-search-select-option.active{background:#111;color:#fff}
      .noc-search-select-empty{padding:16px 10px;text-align:center;color:#7b7c77;font-size:12px}
      @media(max-width:700px){
        .noc-search-select-trigger{min-height:46px}
        .noc-search-select-panel{position:fixed;left:12px;right:12px;top:auto;bottom:12px;border-radius:16px;max-height:72vh}
        .noc-search-select-search-wrap{padding:12px}
        .noc-search-select-search{min-height:46px;font-size:16px}
        .noc-search-select-options{max-height:52vh;padding:7px}
        .noc-search-select-option{min-height:44px;padding:11px 12px;font-size:14px}
      }
    `;
    document.head.appendChild(style);
  }

  function cerrarSelectoresBuscables(except=null){
    document.querySelectorAll(".noc-search-select.open").forEach(w=>{
      if(w===except)return;
      w.classList.remove("open");
      w.querySelector(".noc-search-select-panel")?.remove();
    });
  }

  function etiquetaSelect(sel){
    const opt=sel.options[sel.selectedIndex];
    return opt?.textContent||sel.options[0]?.textContent||"Seleccionar…";
  }

  function sincronizarSelectorBuscable(id){
    const sel=document.getElementById(id);
    const wrap=sel?.closest(".noc-search-select");
    if(!sel||!wrap)return;
    const label=wrap.querySelector(".noc-search-select-label");
    if(label)label.textContent=etiquetaSelect(sel);
  }

  function sincronizarSelectoresBuscables(){
    document.querySelectorAll("select.noc-search-select-native").forEach(sel=>sincronizarSelectorBuscable(sel.id));
  }

  function abrirSelectorBuscable(id){
    const sel=document.getElementById(id);
    const wrap=sel?.closest(".noc-search-select");
    if(!sel||!wrap)return;
    if(wrap.classList.contains("open")){cerrarSelectoresBuscables();return}
    cerrarSelectoresBuscables(wrap);
    wrap.classList.add("open");

    const panel=document.createElement("div");
    panel.className="noc-search-select-panel";
    panel.innerHTML=`
      <div class="noc-search-select-search-wrap">
        <input class="noc-search-select-search" type="search" placeholder="Escribe para filtrar…" autocomplete="off">
      </div>
      <div class="noc-search-select-options"></div>`;
    wrap.appendChild(panel);

    const input=panel.querySelector(".noc-search-select-search");
    const box=panel.querySelector(".noc-search-select-options");

    const pintar=()=>{
      const q=normalizarBusqueda(input.value);
      const opts=[...sel.options].filter(o=>!q||normalizarBusqueda(o.textContent).includes(q));
      box.innerHTML=opts.length?opts.map(o=>`
        <button type="button" class="noc-search-select-option ${String(o.value)===String(sel.value)?"active":""}"
          data-value="${NOC.App.esc(o.value)}">${NOC.App.esc(o.textContent)}</button>`).join("")
        :`<div class="noc-search-select-empty">No hay resultados para “${NOC.App.esc(input.value)}”.</div>`;
      box.querySelectorAll(".noc-search-select-option").forEach(btn=>btn.addEventListener("click",()=>{
        sel.value=btn.dataset.value;
        sel.dispatchEvent(new Event("change",{bubbles:true}));
        sincronizarSelectorBuscable(id);
        cerrarSelectoresBuscables();
      }));
    };
  ...
  function convertirSelectBuscable(id){
    asegurarEstiloSelectoresBuscables();
    const sel=document.getElementById(id);
    if(!sel||sel.classList.contains("noc-search-select-native"))return;
    const wrap=document.createElement("div");
    wrap.className="noc-search-select";
    sel.parentNode.insertBefore(wrap,sel);
    wrap.appendChild(sel);
    sel.classList.add("noc-search-select-native");

    const trigger=document.createElement("button");
    trigger.type="button";
    trigger.className="noc-search-select-trigger";
    trigger.innerHTML=`<span class="noc-search-select-label"></span><span class="noc-search-select-chevron">▼</span>`;
    trigger.addEventListener("click",()=>abrirSelectorBuscable(id));
    wrap.appendChild(trigger);
    sincronizarSelectorBuscable(id);
  }

  function prepararSelectoresBuscables(ids){
    (ids||[]).forEach(convertirSelectBuscable);
  }

  if(!window._nocSearchSelectOutsideBound){
    window._nocSearchSelectOutsideBound=true;
    document.addEventListener("click",e=>{
      if(!e.target.closest(".noc-search-select"))cerrarSelectoresBuscables();
    });
    document.addEventListener("keydown",e=>{if(e.key==="Escape")cerrarSelectoresBuscables()});
  }
  ...
  function dibujarResultadoGuardado(){
    ...
    if(r==="clientes_historial")return drawClientesHistorialResult();
    ...
    if(r==="articulos_detalle")return drawArticulosDetalleResult();
  ...
  function restaurarEstadoInforme(){
    try{
      ...
      sincronizarSelectoresBuscables();
  ...
  async function render(){
      ...
      <div class="grid noc-view noc-view-informes">
        <div class="card col-12">
          <div class="report-tabs">
            ...
            <button class="btn ${currentReport==="clientes_historial"?"btn-primary":""}" onclick="NOC.Informes.cambiarInforme('clientes_historial')">Clientes</button>
            <button class="btn ${currentReport==="clientes"?"btn-primary":""}" onclick="NOC.Informes.cambiarInforme('clientes')">Inactividad de clientes</button>
    ...
    else if(currentReport==="clientes_historial")await drawClientesHistorialFilters();
  ...
  function drawCommercialFilters(comerciales){
    ...
    document.getElementById("reportFilters").innerHTML=`
      ...
      <form id="reportCommercialForm" class="form-grid" onsubmit="event.preventDefault();NOC.Informes.generarComercial()">
        ...
        <div class="field f3 noc-switch-field">
          <label class="noc-switch-label" for="repIgnorarCierre">
            <span class="noc-switch-copy">
              <strong>Ignorar fecha de cierre</strong>
              <small>Mostrar el periodo completo</small>
            </span>
            <span class="noc-switch">
              <input id="repIgnorarCierre" type="checkbox">
              <span class="noc-switch-track"><span class="noc-switch-knob"></span></span>
            </span>
          </label>
        </div>
        <div class="f12 noc-report-actions">
          <button type="button" class="noc-secondary-action" onclick="NOC.Informes.abrirCierresComerciales()">
            <span class="noc-secondary-action-icon">↗</span>
            <span>Control de registros comerciales</span>
          </button>
          <button class="btn btn-primary noc-generate-report" type="submit">Generar informe</button>
        </div>
      </form>`;
    prepararSelectoresBuscables(["repComercial"]);
  }
  ...
  async function drawClientesHistorialFilters(){
    let clientes=[];
    try{
      clientes=await NOC.API.list("clientes",{select:"id,nombre_tienda,nombre,apellidos",order:"nombre_tienda",ascending:true});
    }catch(e){}
    document.getElementById("reportFilters").innerHTML=`
      <h2>Informe de Clientes</h2>
      <p class="muted">Consulta qué ha comprado o pedido un cliente en el periodo indicado. Elige Facturas o Proformas para no mezclar ambos orígenes.</p>
      <form id="reportClientesHistorialForm" class="form-grid" onsubmit="event.preventDefault();NOC.Informes.generarClientesHistorial()">
        <div class="field f4">
          <label>Cliente</label>
          <select id="cliHistCliente" required>
            <option value="">Seleccionar cliente…</option>
            ${(clientes||[]).map(c=>{
              const nombre=c.nombre_tienda||[c.nombre,c.apellidos].filter(Boolean).join(" ")||"Sin nombre";
              return `<option value="${NOC.App.esc(c.id)}">${NOC.App.esc(nombre)}</option>`;
            }).join("")}
          </select>
        </div>
        <div class="field f2"><label>Desde</label><input id="cliHistDesde" type="date" required></div>
        <div class="field f2"><label>Hasta</label><input id="cliHistHasta" type="date" required></div>
        <div class="field f2">
          <label>Consultar</label>
          <select id="cliHistTipo">
            <option value="facturas">Facturas</option>
            <option value="proformas">Proformas</option>
          </select>
        </div>
        <div class="f2 noc-report-actions" style="align-items:end">
          <button class="btn btn-primary noc-generate-report" type="submit">Generar informe</button>
        </div>
      </form>`;
    prepararSelectoresBuscables(["cliHistCliente"]);
  }

  async function generarClientesHistorial(){
    const clienteId=document.getElementById("cliHistCliente").value;
    const clienteSel=document.getElementById("cliHistCliente");
    const cliente=clienteSel?.options[clienteSel.selectedIndex]?.textContent||"";
    const desde=document.getElementById("cliHistDesde").value;
    const hasta=document.getElementById("cliHistHasta").value;
    const tipo=document.getElementById("cliHistTipo").value;
    if(!clienteId||!desde||!hasta)return NOC.App.alertMessage("Faltan datos","Selecciona cliente y ambas fechas.","error");
    ...
    const esFactura=tipo==="facturas";
    const tablaDocs=esFactura?"facturas":"proformas";
    const tablaLineas=esFactura?"lineas_factura":"lineas_proforma";
    const fk=esFactura?"factura_id":"proforma_id";
    const etiqueta=esFactura?"Facturas":"Proformas";
    NOC.App.showProgress("Generando informe…",`${cliente} · ${etiqueta}`);

    try{
      let qDocs=NOC.API.db().from(tablaDocs)
        .select(esFactura
          ?"id,numero,fecha,base_imponible,iva,recargo,total"
          :"id,numero,fecha,estado,base_imponible,iva,recargo,total")
        .eq("cliente_id",clienteId).gte("fecha",desde).lte("fecha",hasta)
        .order("fecha",{ascending:true}).order("numero",{ascending:true});
      if(!esFactura)qDocs=qDocs.neq("estado","Cancelada");
      const {data:docs,error:docsError}=await qDocs;
      if(docsError)throw docsError;

      const ids=(docs||[]).map(d=>d.id);
      let lineas=[];
      if(ids.length){
        const {data,error}=await NOC.API.db().from(tablaLineas)
          .select(`id,${fk},articulo_id,descripcion,precio_unitario,cantidad,descuento,tallaje,orden,es_envio`)
          .in(fk,ids).eq("es_envio",false).order("orden",{ascending:true});
        if(error)throw error;
        lineas=data||[];
      }

      const docsMap=new Map((docs||[]).map(d=>[String(d.id),d]));
      currentRows=lineas.map(l=>{
        const d=docsMap.get(String(l[fk]))||{};
        const cantidad=Number(l.cantidad||0),precio=Number(l.precio_unitario||0),descuento=Number(l.descuento||0);
        return {
          fecha:d.fecha||"",
          documento:d.numero||"",
          estado:esFactura?"":(d.estado||""),
          articulo:l.descripcion||"",
          tallaje:l.tallaje||"",
          cantidad,precio,descuento,
          importe:Math.round((precio*cantidad*(1-descuento/100)+Number.EPSILON)*100)/100
        };
      }).sort((a,b)=>String(a.fecha).localeCompare(String(b.fecha))||String(a.documento).localeCompare(String(b.documento),"es",{numeric:true})||String(a.articulo).localeCompare(String(b.articulo),"es"));

      currentCriteria={
        report:"clientes_historial",clienteId,cliente,desde,hasta,tipo,etiqueta,
        docs:docs||[],orden:"fecha",dir:"asc"
      };
      NOC.App.hideProgress();
      drawClientesHistorialResult();
    }catch(e){
      NOC.App.hideProgress();
      NOC.App.alertMessage("Error al generar informe","No se ha podido obtener el historial del cliente. "+(e.message||e),"error");
    }
  }

  function clienteHistValue(r,key){
    return {
      fecha:r.fecha||"",documento:r.documento||"",estado:r.estado||"",articulo:r.articulo||"",tallaje:r.tallaje||"",
      cantidad:Number(r.cantidad||0),precio:Number(r.precio||0),descuento:Number(r.descuento||0),importe:Number(r.importe||0)
    }[key];
  }

  function ordenarClientesHistorial(key){
    if(!currentCriteria||currentCriteria.report!=="clientes_historial")return;
    ...
    const numeric=["cantidad","precio","descuento","importe"].includes(key);
    currentRows=[...currentRows].sort((a,b)=>{
      const av=clienteHistValue(a,key),bv=clienteHistValue(b,key);
  ...
  function drawClientesHistorialResult(){
    const box=document.getElementById("reportResult"),c=currentCriteria;
    const docs=Array.isArray(c.docs)?c.docs:[];
    const t=totals(docs);
    const unidades=currentRows.reduce((a,r)=>a+Number(r.cantidad||0),0);
    const importeArticulos=currentRows.reduce((a,r)=>a+Number(r.importe||0),0);
    const esFactura=c.tipo==="facturas";
    const cols=[
      ["fecha","Fecha",""],["documento",esFactura?"Factura":"Proforma",""],["articulo","Artículo",""],
      ["tallaje","Tallaje",""],["cantidad","Uds.","num"],["precio","Precio","num"],["descuento","Dto. %","num"],["importe","Importe","num"]
    ];
    if(!esFactura)cols.splice(2,0,["estado","Estado",""]);
    ...
    box.innerHTML=`
      <div class="report-head">
        <div>
          ...
          <h2 style="margin:2px 0 5px">Cliente · ${NOC.App.esc(c.cliente)}</h2>
          <div class="muted">${c.etiqueta} · ${dateEs(c.desde)} a ${dateEs(c.hasta)}${!esFactura?" · Proformas canceladas excluidas":""}</div>
        </div>
      ...
      <div class="article-report-metrics">
        <div class="dash-metric-card"><div class="dash-metric-label">${esFactura?"Facturas":"Proformas"}</div><div class="dash-metric-value">${docs.length}</div></div>
        <div class="dash-metric-card"><div class="dash-metric-label">Total documentos</div><div class="dash-metric-value">${NOC.App.money(t.total)}</div></div>
        <div class="dash-metric-card"><div class="dash-metric-label">Unidades artículos</div><div class="dash-metric-value">${unidades.toLocaleString("es-ES")}</div></div>
        <div class="dash-metric-card"><div class="dash-metric-label">Importe artículos</div><div class="dash-metric-value">${NOC.App.money(importeArticulos)}</div></div>
      </div>

      <h3 style="margin:20px 0 8px">Documentos</h3>
      <div class="table-wrap report-table-wrap"><table class="report-table">
        <thead><tr><th>Fecha</th><th>Nº ${esFactura?"factura":"proforma"}</th>${!esFactura?"<th>Estado</th>":""}<th class="num">Base</th><th class="num">IVA</th><th class="num">RE</th><th class="num">Total</th></tr></thead>
        <tbody>${docs.map(d=>`<tr><td>${dateEs(d.fecha)}</td><td><strong>${NOC.App.esc(d.numero)}</strong></td>${!esFactura?`<td>${NOC.App.esc(d.estado||"")}</td>`:""}<td class="num">${NOC.App.money(d.base_imponible)}</td><td class="num">${NOC.App.money(d.iva)}</td><td class="num">${NOC.App.money(d.recargo)}</td><td class="num"><strong>${NOC.App.money(d.total)}</strong></td></tr>`).join("")||`<tr><td colspan="${esFactura?6:7}" class="empty">No hay ${c.etiqueta.toLowerCase()} para este cliente en el periodo seleccionado.</td></tr>`}</tbody>
        ${docs.length?`<tfoot><tr class="report-total-row"><td colspan="${esFactura?2:3}"><strong>TOTALES</strong></td><td class="num"><strong>${NOC.App.money(t.base)}</strong></td><td class="num"><strong>${NOC.App.money(t.iva)}</strong></td><td class="num"><strong>${NOC.App.money(t.re)}</strong></td><td class="num"><strong>${NOC.App.money(t.total)}</strong></td></tr></tfoot>`:""}
      </table></div>

      <h3 style="margin:22px 0 8px">Artículos</h3>
      <div class="table-wrap report-table-wrap"><table class="report-table">
        <thead><tr>${cols.map(([k,l,cl])=>{const a=c.orden===k;return `<th class="sortable-th ${a?"sort-active":""} ${cl}" onclick="NOC.Informes.ordenarClientesHistorial('${k}')">${NOC.App.esc(l)} <span class="sort-mark">${a?(c.dir==="asc"?"↑":"↓"):"↕"}</span></th>`}).join("")}</tr></thead>
        <tbody>${currentRows.map(r=>`<tr>${cols.map(([k])=>{
          if(k==="fecha")return `<td>${dateEs(r.fecha)}</td>`;
          if(k==="cantidad")return `<td class="num">${Number(r.cantidad||0).toLocaleString("es-ES")}</td>`;
          if(k==="precio"||k==="importe")return `<td class="num">${NOC.App.money(r[k])}</td>`;
          if(k==="descuento")return `<td class="num">${Number(r.descuento||0).toLocaleString("es-ES")}%</td>`;
          return `<td>${NOC.App.esc(r[k]||"")}</td>`;
        }).join("")}</tr>`).join("")||`<tr><td colspan="${cols.length}" class="empty">No hay artículos en los documentos del periodo.</td></tr>`}</tbody>
      </table></div>`;
  ...
  async function drawClientesFilters(){
    ...
    document.getElementById("reportFilters").innerHTML=`
      <h2>Inactividad de clientes</h2>
      <p class="muted">Consulta la cartera y localiza clientes según su última actividad. Las fechas Desde/Hasta son inclusivas.</p>
    ...
    prepararSelectoresBuscables(["cliRepLocalidad","cliRepProvincia","cliRepComercial"]);
  }
  ...
  function drawClientesResult(){
    ...
    box.innerHTML=`
      <div class="report-head">
        <div><div class="report-brand">NOC THE BRAND</div><h2 style="margin:2px 0 5px">Inactividad de clientes</h2>
  ...
  async function drawProformasFilters(){
    ...
    prepararSelectoresBuscables(["profRepTienda","profRepNumDesde","profRepNumHasta"]);
  }
  ...
  async function drawFiscal3000Filters(){
    ...
    document.getElementById("reportFilters").innerHTML=`
      <h2>Informe Modelo 347 · Operaciones con terceros</h2>
      <p class="muted">Muestra las tiendas cuyo total anual facturado supera 3.005,06 € (impuestos incluidos), con desglose por trimestre y detalle de facturas.</p>
      <form class="form-grid" onsubmit="event.preventDefault();NOC.Informes.generarFiscal3000()">
        ...
        <div class="field f3"><label>Umbral anual</label><input type="text" value="3.005,06 € · Total con impuestos" disabled></div>
        <div class="f6 toolbar" style="align-items:end"><div></div><button class="btn btn-primary" type="submit">Generar informe 347</button></div>
      </form>`;
  }

  async function generarFiscal3000(){
    const year=document.getElementById("fiscal3000Year").value;
    const minimo=3005.06,desde=`${year}-01-01`,hasta=`${year}-12-31`;
    NOC.App.showProgress("Generando informe 347…",`Operaciones con terceros · ${year}`);
    try{
      const {data,error}=await NOC.API.db().from("facturas")
        .select("id,numero,fecha,cliente_id,base_imponible,iva,recargo,total,clientes(nombre_tienda,nombre,apellidos,dni_cif,localidad_facturacion,provincia_facturacion)")
        .gte("fecha",desde).lte("fecha",hasta)
        .order("fecha",{ascending:true}).order("numero",{ascending:true});
      if(error)throw error;
      const map=new Map();
      (data||[]).filter(r=>String(r.clientes?.nombre_tienda||"").trim().toLocaleLowerCase("es")!=="particular").forEach(r=>{
        const key=r.cliente_id||r.clientes?.dni_cif||r.clientes?.nombre_tienda;
        if(!map.has(key))map.set(key,{
          tienda:r.clientes?.nombre_tienda||"",cliente:[r.clientes?.nombre,r.clientes?.apellidos].filter(Boolean).join(" "),
          cif:r.clientes?.dni_cif||"",localidad:r.clientes?.localidad_facturacion||"",provincia:r.clientes?.provincia_facturacion||"",
          base:0,iva:0,re:0,total:0,facturas:0,
          trimestres:[1,2,3,4].map(n=>({n,base:0,iva:0,re:0,total:0,facturas:[]}))
        });
        const x=map.get(key),m=Number(String(r.fecha||"").slice(5,7)||1),q=Math.min(4,Math.max(1,Math.floor((m-1)/3)+1)),tr=x.trimestres[q-1];
        const f={numero:r.numero||"",fecha:r.fecha||"",base:Number(r.base_imponible||0),iva:Number(r.iva||0),re:Number(r.recargo||0),total:Number(r.total||0)};
        x.base+=f.base;x.iva+=f.iva;x.re+=f.re;x.total+=f.total;x.facturas++;
        tr.base+=f.base;tr.iva+=f.iva;tr.re+=f.re;tr.total+=f.total;tr.facturas.push(f);
      });
      currentRows=[...map.values()].filter(r=>Math.abs(Number(r.total||0))>minimo).sort((a,b)=>b.total-a.total);
      currentCriteria={report:"fiscal3000",year,minimo,orden:"total",dir:"desc"};
      NOC.App.hideProgress(); drawFiscal3000Result();
    }catch(e){
      NOC.App.hideProgress();
      NOC.App.alertMessage("Error al generar informe","No se ha podido generar el informe 347. "+(e.message||e),"error");
    }
  }

  function fiscal3000Value(r,key){
    return {tienda:r.tienda||"",cliente:r.cliente||"",cif:r.cif||"",localidad:r.localidad||"",provincia:r.provincia||"",facturas:Number(r.facturas||0),base:Number(r.base||0),iva:Number(r.iva||0),re:Number(r.re||0),total:Number(r.total||0)}[key];
  }
  ...
  function drawFiscal3000Result(){
    ...
    box.innerHTML=`
      <div class="report-head">
        <div><div class="report-brand">NOC THE BRAND</div><h2 style="margin:2px 0 5px">Informe Modelo 347 · ${NOC.App.esc(c.year)}</h2><div class="muted">Tiendas con operaciones anuales superiores a ${NOC.App.money(c.minimo)} · Total con impuestos</div></div>
        <div class="toolbar-right"><button class="btn" onclick="NOC.Informes.exportCsv()">Exportar CSV</button><button class="btn btn-primary" onclick="NOC.Informes.imprimir()">Imprimir / Guardar PDF</button></div>
      </div>
      ${currentRows.map(r=>`
        <section style="border:1px solid #e2e2df;border-radius:14px;margin:14px 0;overflow:hidden">
          <div style="padding:14px 16px;background:#111;color:#fff;display:flex;justify-content:space-between;gap:14px;flex-wrap:wrap">
            <div><strong>${NOC.App.esc(r.tienda)}</strong><div style="font-size:11px;opacity:.72;margin-top:3px">${NOC.App.esc(r.cif)} · ${NOC.App.esc(r.localidad)}${r.provincia?` · ${NOC.App.esc(r.provincia)}`:""}</div></div>
            <div style="text-align:right"><small style="opacity:.7">TOTAL ANUAL</small><div style="font-size:18px;font-weight:800">${NOC.App.money(r.total)}</div></div>
          </div>
          <div class="table-wrap"><table class="report-table">
            <thead><tr><th>Trimestre</th><th>Facturas</th><th class="num">Base</th><th class="num">IVA</th><th class="num">RE</th><th class="num">Total</th></tr></thead>
            <tbody>${r.trimestres.map(tr=>`<tr>
              <td><strong>${tr.n}T</strong></td>
              <td>${tr.facturas.length?tr.facturas.map(f=>`<span style="display:inline-block;margin:2px 6px 2px 0;white-space:nowrap"><strong>${NOC.App.esc(f.numero)}</strong> <small class="muted">${dateEs(f.fecha)} · ${NOC.App.money(f.total)}</small></span>`).join(""):"—"}</td>
              <td class="num">${NOC.App.money(tr.base)}</td><td class="num">${NOC.App.money(tr.iva)}</td><td class="num">${NOC.App.money(tr.re)}</td><td class="num"><strong>${NOC.App.money(tr.total)}</strong></td>
            </tr>`).join("")}</tbody>
            <tfoot><tr class="report-total-row"><td colspan="2"><strong>TOTAL ANUAL · ${r.facturas} factura(s)</strong></td><td class="num"><strong>${NOC.App.money(r.base)}</strong></td><td class="num"><strong>${NOC.App.money(r.iva)}</strong></td><td class="num"><strong>${NOC.App.money(r.re)}</strong></td><td class="num"><strong>${NOC.App.money(r.total)}</strong></td></tr></tfoot>
          </table></div>
        </section>`).join("")||`<div class="empty">No hay tiendas que superen ${NOC.App.money(c.minimo)} en ${NOC.App.esc(c.year)}.</div>`}
      <div class="report-count muted">${currentRows.length} tienda(s) incluida(s)</div>`;
    guardarEstadoInforme(); box.scrollIntoView({behavior:"smooth",block:"start"});
  }
  ...
  async function drawArticulosFilters(){
    ...
    document.getElementById("reportFilters").innerHTML=`
      ...
      <p class="muted">Consulta el resumen de artículos o el detalle de en qué tiendas y proformas se pidió un artículo concreto. Las proformas canceladas no cuentan.</p>
      <form id="reportArticlesForm" class="form-grid" onsubmit="event.preventDefault();NOC.Informes.generarArticulos()">
        <div class="field f3"><label>Vista</label><select id="artVista" onchange="NOC.Informes.actualizarVistaArticulos()"><option value="resumen">Resumen</option><option value="detalle">Detalle por artículo</option></select></div>
    ...
    prepararSelectoresBuscables(["artArticulo","artFabrica","artTipo"]);
  }

  function actualizarVistaArticulos(){
    const vista=document.getElementById("artVista")?.value||"resumen";
    const sel=document.getElementById("artArticulo");
    if(!sel)return;
    const first=sel.querySelector('option[value=""]');
    if(first)first.textContent=vista==="detalle"?"Selecciona un artículo…":"Todos";
    sel.required=vista==="detalle";
    sincronizarSelectorBuscable("artArticulo");
  }
  ...
  async function generarArticulos(){
    ...
    if(vista==="detalle"&&!articuloId)return NOC.App.alertMessage("Selecciona un artículo","Para el detalle debes elegir un artículo concreto.","info");
    ...
    try{
      ...
      let q=NOC.API.db().from("lineas_proforma")
        .select("id,proforma_id,articulo_id,descripcion,cantidad,precio_unitario,descuento,es_envio,proformas!inner(fecha,numero,estado,cliente_id,clientes(nombre_tienda,nombre,apellidos))")
        .eq("es_envio",false)
      ...
      if(vista==="detalle"){
        const art=catalogoPorId.get(String(articuloId))||{};
        currentRows=rows.map(r=>{
          const cantidad=Number(r.cantidad||0),precio=Number(r.precio_unitario||0),descuento=Number(r.descuento||0);
          return {
            fecha:r.proformas?.fecha||"",
            proforma:r.proformas?.numero||"",
            tienda:r.proformas?.clientes?.nombre_tienda||[r.proformas?.clientes?.nombre,r.proformas?.clientes?.apellidos].filter(Boolean).join(" ")||"",
            cantidad,precio,descuento,
            importe:precio*cantidad*(1-descuento/100)
          };
        }).sort((a,b)=>String(a.fecha).localeCompare(String(b.fecha))||String(a.proforma).localeCompare(String(b.proforma),"es",{numeric:true}));
        currentCriteria={report:"articulos_detalle",desde,hasta,articuloId,articulo:art.nombre_producto||"",sku:art.sku||"",orden:"fecha",dir:"asc"};
        NOC.App.hideProgress();
        drawArticulosDetalleResult();
        return;
      }
  ...
  function articuloDetalleValue(r,key){
    return {fecha:r.fecha||"",proforma:r.proforma||"",tienda:r.tienda||"",cantidad:Number(r.cantidad||0),precio:Number(r.precio||0),descuento:Number(r.descuento||0),importe:Number(r.importe||0)}[key];
  }
  function ordenarArticulosDetalleResultado(key){
    if(!currentCriteria||currentCriteria.report!=="articulos_detalle")return;
    if(currentCriteria.orden===key)currentCriteria.dir=currentCriteria.dir==="asc"?"desc":"asc";
    else{currentCriteria.orden=key;currentCriteria.dir="asc"}
    const numeric=["cantidad","precio","descuento","importe"].includes(key);
    currentRows=[...currentRows].sort((a,b)=>{
      const av=articuloDetalleValue(a,key),bv=articuloDetalleValue(b,key);
      const cmp=numeric?Number(av)-Number(bv):String(av??"").localeCompare(String(bv??""),"es",{numeric:true,sensitivity:"base"});
      return currentCriteria.dir==="desc"?-cmp:cmp;
    });
    drawArticulosDetalleResult();
  }
  function drawArticulosDetalleResult(){
    const box=document.getElementById("reportResult"),c=currentCriteria;
    const unidades=currentRows.reduce((a,r)=>a+Number(r.cantidad||0),0);
    const importe=currentRows.reduce((a,r)=>a+Number(r.importe||0),0);
    const tiendas=new Set(currentRows.map(r=>r.tienda).filter(Boolean)).size;
    const proformas=new Set(currentRows.map(r=>r.proforma).filter(Boolean)).size;
    const head=[["fecha","Fecha",""],["proforma","Proforma",""],["tienda","Tienda",""],["cantidad","Cantidad","num"],["precio","Precio","num"],["descuento","Dto. %","num"],["importe","Importe","num"]];
    box.style.display="";
    box.innerHTML=`
      <div class="report-head">
        <div><div class="report-brand">NOC THE BRAND</div><h2 style="margin:2px 0 5px">Detalle · ${NOC.App.esc(c.articulo)}</h2><div class="muted">${NOC.App.esc(c.sku||"")}${c.desde?` · Desde ${dateEs(c.desde)}`:""}${c.hasta?` · Hasta ${dateEs(c.hasta)}`:""}</div></div>
        <div class="toolbar-right"><button class="btn" onclick="NOC.Informes.exportCsv()">Exportar CSV</button><button class="btn btn-primary" onclick="NOC.Informes.imprimir()">Imprimir / Guardar PDF</button></div>
      </div>
      <div style="display:flex;gap:10px;flex-wrap:wrap;margin:10px 0 14px">
        <span class="badge">${tiendas} tienda(s)</span><span class="badge">${proformas} proforma(s)</span><span class="badge">${unidades.toLocaleString("es-ES")} uds.</span><span class="badge">${NOC.App.money(importe)}</span>
      </div>
      <div class="table-wrap"><table class="report-table">
        <thead><tr>${head.map(([k,l,cl])=>{const a=c.orden===k;return `<th class="sortable-th ${a?"sort-active":""} ${cl}" onclick="NOC.Informes.ordenarArticulosDetalleResultado('${k}')">${l} <span class="sort-mark">${a?(c.dir==="asc"?"↑":"↓"):"↕"}</span></th>`}).join("")}</tr></thead>
        <tbody>${currentRows.map(r=>`<tr><td>${dateEs(r.fecha)}</td><td><strong>${NOC.App.esc(r.proforma)}</strong></td><td>${NOC.App.esc(r.tienda)}</td><td class="num">${Number(r.cantidad).toLocaleString("es-ES")}</td><td class="num">${NOC.App.money(r.precio)}</td><td class="num">${Number(r.descuento||0).toLocaleString("es-ES")}%</td><td class="num"><strong>${NOC.App.money(r.importe)}</strong></td></tr>`).join("")||`<tr><td colspan="7" class="empty">No hay pedidos de este artículo en el periodo seleccionado.</td></tr>`}</tbody>
        ${currentRows.length?`<tfoot><tr class="report-total-row"><td colspan="3"><strong>TOTALES</strong></td><td class="num"><strong>${unidades.toLocaleString("es-ES")}</strong></td><td></td><td></td><td class="num"><strong>${NOC.App.money(importe)}</strong></td></tr></tfoot>`:""}
      </table></div>`;
    guardarEstadoInforme(); box.scrollIntoView({behavior:"smooth",block:"start"});
  }
  ...
  function asegurarEstiloCierres(){
    ...
    style.textContent=`
      ...
      .noc-switch-copy strong{
        font-size:12px;
        font-weight:750;
        letter-spacing:-.01em;
        color:#111;
      }
      .noc-switch-copy small{
        font-size:10px;
        font-weight:500;
        color:#777872;
      }
      ...
      .noc-switch input:checked + .noc-switch-track{
        background:#0d0e0e;
      }
      .noc-switch input:checked + .noc-switch-track .noc-switch-knob{
        transform:translateX(20px);
      }
      .noc-switch input:focus-visible + .noc-switch-track{
      ...
      .noc-secondary-action:hover{
        background:#f5f5f2;
        border-color:#bdbdb8;
      }
      .noc-secondary-action:active{transform:translateY(1px)}
      .noc-secondary-action-icon{
        width:21px;
        height:21px;
        display:grid;
        place-items:center;
      ...
      @media(max-width:700px){
        .noc-switch-field{padding-top:4px}
        .noc-switch-label{
          min-height:58px;
          padding:10px 12px 10px 14px;
        }
        .noc-switch-copy strong{font-size:12px}
        .noc-switch-copy small{font-size:10px}
        .noc-report-actions{
          flex-direction:column;
          align-items:stretch;
          gap:9px;
        ...
        .noc-secondary-action,.noc-generate-report{
          width:100%;
    ...
    try{
      let q=NOC.API.db().from(tabla)
        .select(tipo==="facturado"
          ?"id,numero,fecha,base_imponible,iva,recargo,total,clientes!inner(nombre_tienda,comercial)"
          :"id,numero,fecha,estado,base_imponible,iva,recargo,total,envio_precio,envio_cantidad,envio_descuento,clientes!inner(nombre_tienda,comercial)")
      ...
      // SIN GASTOS DE ENVÍO:
      // - Proformas: el porte contractual está guardado en la propia cabecera
      //   (envio_precio / envio_descuento), incluso en documentos históricos
      //   que no tienen una línea es_envio.
      // - Facturas: no existen esos campos; el porte se recupera de la línea
      //   de factura marcada es_envio=true.
      const envioPorDocumento=new Map();
      if(!incluirEnvio&&(data||[]).length&&tipo==="facturado"){
        const ids=(data||[]).map(r=>r.id);
        const {data:lineasEnvio,error:envioError}=await NOC.API.db().from("lineas_factura")
          .select("factura_id,precio_unitario,cantidad,descuento,es_envio")
          .in("factura_id",ids).eq("es_envio",true);
        if(envioError)throw envioError;
        (lineasEnvio||[]).forEach(l=>{
          const neto=Number(l.precio_unitario||0)*Number(l.cantidad||0)*(1-Number(l.descuento||0)/100);
          const id=String(l.factura_id);
          envioPorDocumento.set(id,(envioPorDocumento.get(id)||0)+neto);
        });
      }
      ...
      currentRows=(data||[]).map(r=>{
        ...
        const reOriginal=Number(r.recargo||0);
        const totalOriginal=Number(r.total||0);

        let envioNeto=0;
        if(!incluirEnvio){
          envioNeto=tipo==="proformado"
            ? Number(r.envio_precio||0)*Math.max(1,Number(r.envio_cantidad||1))*(1-Number(r.envio_descuento||0)/100)
            : Number(envioPorDocumento.get(String(r.id))||0);
        }

        const baseNueva=incluirEnvio
          ? baseOriginal
          : Math.max(0,Math.round((baseOriginal-envioNeto+Number.EPSILON)*100)/100);

        // El informe debe mostrar importes coherentes CON/SIN porte, no solo
        // cambiar la Base dejando IVA/RE/Total antiguos.
        // Conservamos los tipos efectivos del documento y los aplicamos a la
        // nueva base (el envío usa la misma fiscalidad del documento).
        const ratio=baseOriginal>0?baseNueva/baseOriginal:1;
        const ivaNueva=incluirEnvio?ivaOriginal:Math.round((ivaOriginal*ratio+Number.EPSILON)*100)/100;
        const reNueva=incluirEnvio?reOriginal:Math.round((reOriginal*ratio+Number.EPSILON)*100)/100;
        const totalNuevo=incluirEnvio?totalOriginal:Math.round((baseNueva+ivaNueva+reNueva+Number.EPSILON)*100)/100;

        return {
          ...r,
          base_imponible_original:baseOriginal,
          iva_original:ivaOriginal,
          recargo_original:reOriginal,
          total_original:totalOriginal,
          envio_informe:envioNeto,
          base_imponible:baseNueva,
          iva:ivaNueva,
          recargo:reNueva,
          total:totalNuevo
        };
  ...
  function exportCsv(){
    ...
    if(currentCriteria.report==="fiscal3000"){
      const headers=["Tienda","Cliente","CIF/NIF","Localidad","Provincia","Trimestre","Factura","Fecha","Base","IVA","RE","Total"];
      const lines=[headers.map(csvCell).join(";")];
      currentRows.forEach(r=>r.trimestres.forEach(tr=>tr.facturas.forEach(f=>lines.push([
        r.tienda,r.cliente,r.cif,r.localidad,r.provincia,`${tr.n}T`,f.numero,dateEs(f.fecha),
        f.base.toFixed(2).replace(".",","),f.iva.toFixed(2).replace(".",","),f.re.toFixed(2).replace(".",","),f.total.toFixed(2).replace(".",",")
      ].map(csvCell).join(";")))));
      downloadBlob(`modelo_347_detalle_${currentCriteria.year}.csv`,"\uFEFF"+lines.join("\r\n"),"text/csv;charset=utf-8");
      return NOC.App.alertMessage("Exportación finalizada",`${currentRows.length} tienda(s) exportadas con detalle trimestral.`,"success");
    }

    if(currentCriteria.report==="articulos_detalle"){
      const headers=["Fecha","Proforma","Tienda","Cantidad","Precio","Dto. %","Importe"];
      const lines=[headers.map(csvCell).join(";")];
      currentRows.forEach(r=>lines.push([dateEs(r.fecha),r.proforma,r.tienda,
        Number(r.cantidad||0).toString().replace(".",","),Number(r.precio||0).toFixed(2).replace(".",","),
        Number(r.descuento||0).toString().replace(".",","),Number(r.importe||0).toFixed(2).replace(".",",")
      ].map(csvCell).join(";")));
      downloadBlob(`detalle_articulo_${currentCriteria.articulo||"articulo"}_${isoToday()}.csv`,"\uFEFF"+lines.join("\r\n"),"text/csv;charset=utf-8");
      return NOC.App.alertMessage("Exportación finalizada",`${currentRows.length} línea(s) exportadas.`,"success");
    }

    if(currentCriteria.report==="articulos"){
      const headers=["Artículo","SKU","Tipo","Fábrica","Unidades pedidas","Importe líneas"];
      const lines=[headers.map(csvCell).join(";")];
  ...
  function imprimir(){
    ...
    if(c.report==="clientes_historial"){
      const docs=Array.isArray(c.docs)?c.docs:[],esFactura=c.tipo==="facturas",t=totals(docs);
      const docRows=docs.map(d=>`<tr><td>${dateEs(d.fecha)}</td><td>${NOC.App.esc(d.numero)}</td>${!esFactura?`<td>${NOC.App.esc(d.estado||"")}</td>`:""}<td class="n">${NOC.App.money(d.base_imponible)}</td><td class="n">${NOC.App.money(d.iva)}</td><td class="n">${NOC.App.money(d.recargo)}</td><td class="n">${NOC.App.money(d.total)}</td></tr>`).join("");
      const artRows=currentRows.map(r=>`<tr><td>${dateEs(r.fecha)}</td><td>${NOC.App.esc(r.documento)}</td>${!esFactura?`<td>${NOC.App.esc(r.estado||"")}</td>`:""}<td>${NOC.App.esc(r.articulo)}</td><td>${NOC.App.esc(r.tallaje||"")}</td><td class="n">${r.cantidad}</td><td class="n">${NOC.App.money(r.precio)}</td><td class="n">${r.descuento}%</td><td class="n">${NOC.App.money(r.importe)}</td></tr>`).join("");
      w.document.write(`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Cliente ${NOC.App.esc(c.cliente)}</title><style>@page{size:A4 landscape;margin:10mm}body{font-family:Arial,sans-serif;font-size:9px;color:#171717}.brand{font-size:11px;font-weight:800;letter-spacing:2px}h1{font-size:18px;margin:4px 0}.meta{color:#555;margin-bottom:14px}h2{font-size:12px;margin:16px 0 6px}table{width:100%;border-collapse:collapse}th{text-align:left;padding:5px;border-bottom:1px solid #bbb}td{padding:5px;border-bottom:1px solid #e5e5e5}.n{text-align:right}tfoot{font-weight:700}</style></head><body><div class="brand">NOC THE BRAND</div><h1>Cliente · ${NOC.App.esc(c.cliente)}</h1><div class="meta">${c.etiqueta} · ${dateEs(c.desde)} a ${dateEs(c.hasta)}</div><h2>Documentos</h2><table><thead><tr><th>Fecha</th><th>Documento</th>${!esFactura?"<th>Estado</th>":""}<th>Base</th><th>IVA</th><th>RE</th><th>Total</th></tr></thead><tbody>${docRows}</tbody><tfoot><tr><td colspan="${esFactura?2:3}">TOTALES</td><td class="n">${NOC.App.money(t.base)}</td><td class="n">${NOC.App.money(t.iva)}</td><td class="n">${NOC.App.money(t.re)}</td><td class="n">${NOC.App.money(t.total)}</td></tr></tfoot></table><h2>Artículos</h2><table><thead><tr><th>Fecha</th><th>Documento</th>${!esFactura?"<th>Estado</th>":""}<th>Artículo</th><th>Tallaje</th><th>Uds.</th><th>Precio</th><th>Dto.</th><th>Importe</th></tr></thead><tbody>${artRows}</tbody></table><script>window.onload=()=>window.print();</script></body></html>`);
      return w.document.close();
    }

    if(c.report==="fiscal3000"){
      const sections=currentRows.map(r=>`<section><h2>${NOC.App.esc(r.tienda)} <span>${NOC.App.esc(r.cif)}</span></h2><div class="annual">Total anual: ${NOC.App.money(r.total)}</div><table><thead><tr><th>Trimestre</th><th>Facturas</th><th class="n">Base</th><th class="n">IVA</th><th class="n">RE</th><th class="n">Total</th></tr></thead><tbody>${r.trimestres.map(tr=>`<tr><td><b>${tr.n}T</b></td><td>${tr.facturas.map(f=>`${NOC.App.esc(f.numero)} · ${dateEs(f.fecha)} · ${NOC.App.money(f.total)}`).join("<br>")||"—"}</td><td class="n">${NOC.App.money(tr.base)}</td><td class="n">${NOC.App.money(tr.iva)}</td><td class="n">${NOC.App.money(tr.re)}</td><td class="n"><b>${NOC.App.money(tr.total)}</b></td></tr>`).join("")}</tbody><tfoot><tr><td colspan="2"><b>TOTAL ANUAL · ${r.facturas} factura(s)</b></td><td class="n">${NOC.App.money(r.base)}</td><td class="n">${NOC.App.money(r.iva)}</td><td class="n">${NOC.App.money(r.re)}</td><td class="n"><b>${NOC.App.money(r.total)}</b></td></tr></tfoot></table></section>`).join("");
      w.document.write(`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Modelo 347 ${c.year}</title><style>@page{size:A4 landscape;margin:10mm}body{font-family:Arial,sans-serif;font-size:9px;color:#171717}.brand{font-size:11px;font-weight:800;letter-spacing:2px}h1{font-size:18px;margin:4px 0}.meta{color:#555;margin-bottom:14px}section{break-inside:avoid;margin:0 0 18px}h2{background:#111;color:#fff;padding:8px 10px;margin:0;font-size:12px}h2 span{font-weight:400;opacity:.7;margin-left:8px}.annual{text-align:right;font-weight:800;padding:5px 0}table{width:100%;border-collapse:collapse}th{text-align:left;padding:5px;border-bottom:1px solid #bbb}td{padding:5px;border-bottom:1px solid #e5e5e5;vertical-align:top}.n{text-align:right}tfoot{font-weight:700}</style></head><body><div class="brand">NOC THE BRAND</div><h1>Informe Modelo 347 · ${c.year}</h1><div class="meta">Operaciones anuales superiores a ${NOC.App.money(c.minimo)} · importe total con impuestos · desglose trimestral</div>${sections}<script>window.onload=()=>window.print();</script></body></html>`);
      return w.document.close();
    }

    if(c.report==="articulos_detalle"){
      const unidades=currentRows.reduce((a,r)=>a+Number(r.cantidad||0),0),importe=currentRows.reduce((a,r)=>a+Number(r.importe||0),0);
      const body=currentRows.map(r=>`<tr><td>${dateEs(r.fecha)}</td><td>${NOC.App.esc(r.proforma)}</td><td>${NOC.App.esc(r.tienda)}</td><td class="n">${r.cantidad}</td><td class="n">${NOC.App.money(r.precio)}</td><td class="n">${r.descuento}%</td><td class="n">${NOC.App.money(r.importe)}</td></tr>`).join("");
      w.document.write(`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Detalle artículo</title><style>@page{size:A4 landscape;margin:10mm}body{font-family:Arial,sans-serif;font-size:9px;color:#171717}.brand{font-size:11px;font-weight:800;letter-spacing:2px}h1{font-size:18px;margin:4px 0}.meta{color:#555;margin-bottom:14px}table{width:100%;border-collapse:collapse}th{text-align:left;padding:6px;border-bottom:1px solid #bbb}td{padding:6px;border-bottom:1px solid #e5e5e5}.n{text-align:right}</style></head><body><div class="brand">NOC THE BRAND</div><h1>Detalle · ${NOC.App.esc(c.articulo)}</h1><div class="meta">${currentRows.length} línea(s) · ${unidades} unidades · ${NOC.App.money(importe)}</div><table><thead><tr><th>Fecha</th><th>Proforma</th><th>Tienda</th><th>Cantidad</th><th>Precio</th><th>Dto.</th><th>Importe</th></tr></thead><tbody>${body}</tbody></table><script>window.onload=()=>window.print();</script></body></html>`);
      return w.document.close();
    }
