...
NOC.Informes=(()=>{
  ...
  function drawProformasResult(){
    ...
    box.innerHTML=`
      ...
      <div class="table-wrap report-table-wrap"><table class="report-table">
        <thead><tr>${cols.map(k=>`<th class="${["base","iva","re","total"].includes(k)?"num":""}">${NOC.App.esc(proformaColumnDefs[k]?.label||k)}</th>`).join("")}</tr></thead>
  ...
  return{render,cambiarInforme,generarComercial,generarVentas,generarProformas,limpiarProformas,generarArticulos,generarFiscal3000,exportCsv,imprimir};
})();