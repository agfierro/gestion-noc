  ...
  async function abrirCierresComerciales(){
    ...
    NOC.App.modal(`
      ...
      <div class="modal-body">
        <p class="muted">Indica hasta qué fecha consideras ya registrado cada comercial. El día de cierre queda incluido; el siguiente informe normal empezará al día siguiente.</p>
        <div style="display:grid;gap:10px">
          ${comerciales.map(c=>{
            const x=map.get(c)||{};
            return `<div style="display:grid;grid-template-columns:1.2fr 180px 1fr auto;gap:10px;align-items:end;border-bottom:1px solid #eee;padding-bottom:10px">
              <div class="field"><label>Comercial</label><input value="${NOC.App.esc(c)}" disabled></div>
              <div class="field"><label>Registrado hasta</label><input id="cierreFecha_${btoa(unescape(encodeURIComponent(c))).replace(/=/g,"")}" type="date" value="${x.fecha_fin_registro||""}"></div>
              <div class="field"><label>Observaciones</label><input id="cierreObs_${btoa(unescape(encodeURIComponent(c))).replace(/=/g,"")}" value="${NOC.App.esc(x.observaciones||"")}" placeholder="Opcional"></div>
              <button class="btn" onclick="NOC.Informes.guardarCierreComercial('${encodeURIComponent(c)}')">Guardar</button>
            </div>`;
