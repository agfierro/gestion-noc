    ...
    try{
      let q=NOC.API.db().from(tabla)
        .select(tipo==="facturado"
          ...
          :"id,numero,fecha,estado,base_imponible,iva,recargo,total,clientes!inner(nombre_tienda,comercial)")
      ...
      // Para "Sin gastos de envío" usamos SIEMPRE la línea real de envío
      // (es_envio=true), tanto en proformas como en facturas. Es la fuente
      // fiable del importe neto del porte realmente incluido en la Base.
      const envioPorDocumento=new Map();
      if(!incluirEnvio&&(data||[]).length){
        const ids=(data||[]).map(r=>r.id);
        const tablaLineas=tipo==="facturado"?"lineas_factura":"lineas_proforma";
        const fk=tipo==="facturado"?"factura_id":"proforma_id";
        const {data:lineasEnvio,error:envioError}=await NOC.API.db().from(tablaLineas)
          .select(`${fk},precio_unitario,cantidad,descuento,es_envio`)
          .in(fk,ids).eq("es_envio",true);
        if(envioError)throw envioError;
        (lineasEnvio||[]).forEach(l=>{
          const neto=Number(l.precio_unitario||0)*Number(l.cantidad||0)*(1-Number(l.descuento||0)/100);
          const id=String(l[fk]);
          envioPorDocumento.set(id,(envioPorDocumento.get(id)||0)+neto);
        });
      }

      currentRows=(data||[]).map(r=>{
        const envioNeto=Number(envioPorDocumento.get(String(r.id))||0);
