...
NOC.Facturas=(()=>{
 ...
 function pdfMode(v){pdfRange.mode=v==="numero"?"numero":"fecha";draw()}
 function pdfField(k,v){pdfRange[k]=v||"";draw()}
 async function generarPdfZip(){
   const sel=pdfRows();
   if(!sel.length)return NOC.App.toast("Selecciona un rango de facturas.");
   const label=pdfRangeLabel().replace(/\s+a\s+/g,"_a_");
 ...
 return{render,ver,search,setSort,openProforma,imprimirActual,pdfMode,pdfField,generarPdfZip}
})();
