...
NOC.Informes=(()=>{
  ...
  async function generarComercialConEnvio(incluirEnvio,ignorarCierre=false){
    ...
    try{
      ...
      // En proformas el porte vive en envio_precio/envio_descuento.
      // En facturas NO existen esas columnas: el porte se guarda como línea
      // de factura con es_envio=true. Recuperamos esas líneas solo cuando
      // el usuario pide el informe SIN gastos de envío.
      const envioPorFactura=new Map();
      if(tipo==="facturado"&&!incluirEnvio&&(data||[]).length){
        ...
        (lineasEnvio||[]).forEach(l=>{
          ...
          envioPorFactura.set(String(l.factura_id),(envioPorFactura.get(String(l.factura_id))||0)+neto);
        });
      }

      currentRows=(data||[]).map(r=>{
        const envioNeto=tipo==="facturado"
          ?Number(envioPorFactura.get(String(r.id))||0)
