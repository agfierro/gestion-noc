...
function taxFor(c){
 ...
 function selectByStatus(status){
   ...
   // Reflejar la selección directamente en la tabla actual.
   document.querySelectorAll(".pf-row-check").forEach(ch=>{
     ch.checked=selectedIds.has(ch.dataset.id);
   });
   const all=document.getElementById("pfCheckAll");
   if(all){
     const visibleChecks=[...document.querySelectorAll(".pf-row-check")];
     all.checked=visibleChecks.length>0&&visibleChecks.every(ch=>ch.checked);
     all.indeterminate=visibleChecks.some(ch=>ch.checked)&&!all.checked;
   }
   const bar=document.getElementById("pfBulkBar");
   if(bar){
     bar.classList.toggle("has-selection",selectedIds.size>0);
