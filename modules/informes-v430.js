...
NOC.Informes=(()=>{
  ...
  async function generarComercial(){
    ...
    NOC.App.modal(`
      ...
      <div class="modal-foot" style="display:flex;gap:8px;justify-content:flex-end">
        <button class="btn" onclick="NOC.Informes.generarComercialConEnvio(false)">Sin gastos de envío</button>
        <button class="btn btn-primary" onclick="NOC.Informes.generarComercialConEnvio(true)">Con gastos de envío</button>
      </div>`,true);
  }

  async function generarComercialConEnvio(incluirEnvio){
    const comercial=document.getElementById("repComercial").value.trim();
    const tipo=document.getElementById("repTipo").value;
    const desde=document.getElementById("repDesde").value;
    const hasta=document.getElementById("repHasta").value;
    ...
    try{
      ...
      currentCriteria={
        report:"comercial",
        comercial,tipo,etiqueta,desde,hasta,
        orden:"fecha",dir:"asc",
  ...
  function drawCommercialResult(){
    ...
    box.innerHTML=`
      <div class="report-head">
        <div><h2 style="margin-bottom:5px">Informe por comercial</h2><div class="muted"><strong>${NOC.App.esc(c.comercial)}</strong> · ${c.etiqueta} · ${dateEs(c.desde)} a ${dateEs(c.hasta)} · ${c.incluirEnvio===false?"Sin gastos de envío":"Con gastos de envío"}</div></div>
        <div class="toolbar-right"><button class="btn" onclick="NOC.Informes.exportCsv()">Exportar CSV</button><button class="btn btn-primary" onclick="NOC.Informes.imprimir()">Imprimir / Guardar PDF</button></div>
      </div>
      <div class="table-wrap report-table-wrap"><table class="report-table">
        <thead><tr>
