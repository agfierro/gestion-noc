...
NOC.Informes=(()=>{
  ...
  const clienteColumnDefs={
    ...
    fechaFactura:{label:"Fecha últ. factura"},
    ultimaActividad:{label:"Última compra"},
    tipoActividad:{label:"Tipo última compra"}
  };
  ...
  async function generarClientes(){
    ...
    try{
      ...
      for(const p of proformas){if(!p.cliente_id)continue;if(!pMap.has(p.cliente_id))pMap.set(p.cliente_id,[]);pMap.get(p.cliente_id).push(p)}
      for(const f of facturas){if(!f.cliente_id)continue;if(!fMap.has(f.cliente_id))fMap.set(f.cliente_id,[]);fMap.get(f.cliente_id).push(f)}
  ...
  return{render,cambiarInforme,generarComercial,generarVentas,generarProformas,limpiarProformas,ordenarProformasResultado,generarClientes,ordenarClientesResultado,generarArticulos,generarFiscal3000,exportCsv,imprimir};
})();