...
NOC.Facturas=(()=>{
 ...
 function imprimirActual(){
   ...
   // Safari es mucho más fiable imprimiendo el DOM que ya está renderizado
   // que una ventana nueva creada con document.write(). Clonamos únicamente
   // la factura y retiramos físicamente el resto de la aplicación del flujo
   // durante la impresión.
   document.getElementById("nocPrintRoot")?.remove();
   document.getElementById("nocPrintStyle")?.remove();

   const root=document.createElement("div");
   root.id="nocPrintRoot";
   ...
   style.id="nocPrintStyle";
   style.textContent=`
     #nocPrintRoot{display:none}
     @media print{
       ...
       body.noc-printing > :not(#nocPrintRoot){display:none!important}
       body.noc-printing > #nocPrintRoot{
       ...
       #nocPrintRoot .doc-sheet.noc-doc{
         display:block!important;
         position:static!important;
         margin:0!important;
         width:210mm!important;
   ...
   const cleanup=()=>{
     document.body.classList.remove("noc-printing");
     root.remove();
     style.remove();
   ...
   document.body.classList.add("noc-printing");
   waitImages.then(()=>{
