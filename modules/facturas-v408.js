...
NOC.Facturas=(()=>{
 ...
 function imprimirActual(){
   const sheet=document.querySelector("#modalRoot .noc-doc");
   if(!sheet)return NOC.App.alertMessage?.("Factura no disponible","Abre primero la factura que quieres imprimir.","info");
   const w=window.open("","_blank","width=1000,height=850");
   if(!w)return NOC.App.alertMessage?.("Ventana bloqueada","Permite ventanas emergentes para imprimir o guardar la factura.","error");
   const base=NOC.App.esc(document.baseURI);
   const title=NOC.App.esc(document.querySelector("#modalRoot .modal-head strong")?.textContent||"Factura NOC");
   w.document.write(`<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><base href="${base}"><title>${title}</title>
   <link rel="stylesheet" href="css/styles.css?v=3.7.1"><link rel="stylesheet" href="css/responsive.css?v=3.9.9"><link rel="stylesheet" href="css/noc-fashion-v383.css?v=4.0.1">
   <style>
     ...
     .doc-sheet.noc-doc{width:210mm!important;max-width:210mm!important;min-height:297mm!important;margin:0!important;padding:12mm 12mm 10mm!important;box-shadow:none!important;break-after:avoid-page!important;page-break-after:avoid!important}
     .noc-doc .doc-items-spacer{height:150px!important}
     *{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
     @media print{html,body{width:210mm!important}.doc-sheet.noc-doc{page-break-inside:avoid!important;break-inside:avoid-page!important}}
   </style></head><body>${sheet.outerHTML}<script>
   (function(){
     function ready(){var imgs=Array.from(document.images);return Promise.all(imgs.map(function(img){return img.complete?Promise.resolve():new Promise(function(res){img.addEventListener('load',res,{once:true});img.addEventListener('error',res,{once:true});setTimeout(res,1800);});}));}
     window.addEventListener('load',function(){ready().then(function(){setTimeout(function(){window.print();},180);});});
   })();
   <\/script></body></html>`);
   w.document.close();
 }
 ...
 return{render,ver,search,setSort,openProforma,imprimirActual}
})();