...
NOC.Documentos=(()=>{
  ...
  async function ajustarDocumentoAPaginas(sheet){
    ...
    const target=Math.max(0,pages*pageH-naturalH-18);
  ...
  async function imprimirActual(){
    ...
    const oldVar=sheet.style.getPropertyValue("--noc-doc-spacer");
    try{
      const h=await medirSpacerA4DesdeClon(sheet);
      if(spacer)spacer.style.height=h;
      sheet.style.setProperty("--noc-doc-spacer",h);
    }catch(e){console.warn("No se pudo medir la paginación A4",e)}
    ...
    const cleanup=()=>{
      ...
      if(spacer)spacer.style.height=oldSpacer;
      if(oldVar)sheet.style.setProperty("--noc-doc-spacer",oldVar);
      else sheet.style.removeProperty("--noc-doc-spacer");
  ...
  function ensurePdfExportStyle(){
    ...
    style.textContent=`
      ...
      .noc-pdf-export-host .noc-doc .doc-items-spacer{height:var(--noc-doc-spacer,0px)!important;min-height:0!important;}
      ...
      .noc-pdf-export-host .noc-doc .doc-items th{padding-top:5px!important;padding-bottom:5px!important;border-color:rgba(0,0,0,.10)!important;}
      .noc-pdf-export-host .noc-doc .doc-items td{padding-top:4px!important;padding-bottom:4px!important;line-height:1.15!important;border-color:rgba(0,0,0,.07)!important;}
      .noc-pdf-export-host .noc-doc .doc-items{border-color:rgba(0,0,0,.08)!important;}
    `;
    document.head.appendChild(style);
    return style;
  }
  ...
  async function pdfBlob({tipo,doc,lineas,config}){
    ...
    try{
      ...
      await ajustarDocumentoAPaginas(sheet);
      const opt={
        margin:0,
