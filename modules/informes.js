...
NOC.Informes=(()=>{
    ...
    document.getElementById("viewContainer").innerHTML=`
      <div class="grid">
        <div class="card col-12">
          ...
          <form id="reportCommercialForm" class="form-grid" onsubmit="event.preventDefault();NOC.Informes.generar()">
            <div class="field f3">
  ...
  async function generar(){
    ...
    if(!comercial||!desde||!hasta){
      return NOC.App.alertMessage("Faltan datos","Selecciona comercial y ambas fechas.","error");
    }
    if(desde>hasta){
      return NOC.App.alertMessage("Periodo incorrecto","La fecha de inicio no puede ser posterior a la fecha fin.","error");
    }
    ...
    try{
      let q=NOC.API.db()
        ...
        .eq("clientes.comercial",comercial)
        .gte("fecha",desde)
        .lte("fecha",hasta)
      ...
      currentRows=data||[];
      currentCriteria={comercial,tipo,etiqueta,desde,hasta};
  ...
  function drawResult(){
    ...
    box.innerHTML=`
      <div class="report-head">
        <div>
          <h2 style="margin-bottom:5px">Informe por comercial</h2>
          <div class="muted">
            <strong>${NOC.App.esc(c.comercial)}</strong> · ${c.etiqueta} ·
            ${dateEs(c.desde)} a ${dateEs(c.hasta)}
          </div>
        </div>
      ...
      <div class="table-wrap report-table-wrap">
        <table class="report-table">
          <thead><tr>
            <th>Fecha</th>
            <th>Nº ${c.tipo==="facturado"?"factura":"proforma"}</th>
            <th>Tienda</th>
          ...
          <tbody>
            ...
            </tr>`).join("") || `<tr><td colspan="7" class="empty">No hay documentos para este comercial en el periodo seleccionado.</td></tr>`}
          </tbody>
      ...
      <div class="report-count muted">${currentRows.length} documento(s)</div>`;
  ...
  function exportCsv(){
    ...
    if(!currentRows.length){
      return NOC.App.alertMessage("Sin datos","No hay registros que exportar.","info");
    }
    const c=currentCriteria,t=totals(currentRows);
    ...
    lines.push([
      ...
      t.iva.toFixed(2).replace(".",","),
      t.re.toFixed(2).replace(".",","),
      t.total.toFixed(2).replace(".",",")
    ].map(csvCell).join(";"));

    const safe=c.comercial.replace(/[^\p{L}\p{N}_-]+/gu,"_");
    downloadBlob(
      `informe_${c.tipo}_${safe}_${c.desde}_${c.hasta}.csv`,
      "\uFEFF"+lines.join("\r\n"),
  ...
  function imprimir(){
    if(!currentCriteria)return;
    if(!currentRows.length){
      return NOC.App.alertMessage("Sin datos","No hay registros que imprimir.","info");
    }
    const c=currentCriteria,t=totals(currentRows);
    ...
    const bodyRows=currentRows.map(r=>`<tr>
      <td>${dateEs(r.fecha)}</td>
      <td>${NOC.App.esc(r.numero)}</td>
      <td>${NOC.App.esc(r.clientes?.nombre_tienda||"")}</td>
      <td class="n">${NOC.App.money(r.base_imponible)}</td>
      <td class="n">${NOC.App.money(r.iva)}</td>
      <td class="n">${NOC.App.money(r.recargo)}</td>
      <td class="n"><strong>${NOC.App.money(r.total)}</strong></td>
    </tr>`).join("");

    w.document.write(`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Informe ${NOC.App.esc(c.comercial)}</title>
      <style>
        @page{size:A4 landscape;margin:14mm}
        body{font-family:Arial,Helvetica,sans-serif;color:#171717;font-size:11px;margin:0}
        h1{font-size:22px;margin:0 0 5px}.meta{color:#555;margin-bottom:20px}
        table{width:100%;border-collapse:collapse}
        th{background:#eef3cf;text-align:left;padding:8px;border:1px solid #d8ddc8;font-size:10px}
        td{padding:8px;border:1px solid #e1e4e7}.n{text-align:right;white-space:nowrap}
        tfoot td{background:#f4f5f6;border-top:2px solid #111;font-weight:700}
        .count{margin-top:9px;color:#666}
      </style></head><body>
      <h1>Informe por comercial</h1>
      <div class="meta"><strong>${NOC.App.esc(c.comercial)}</strong> · ${c.etiqueta} · ${dateEs(c.desde)} a ${dateEs(c.hasta)}</div>
      <table>
        <thead><tr><th>Fecha</th><th>Nº ${c.tipo==="facturado"?"factura":"proforma"}</th><th>Tienda</th><th>Base</th><th>IVA</th><th>RE</th><th>Total</th></tr></thead>
        <tbody>${bodyRows}</tbody>
        <tfoot><tr><td colspan="3">TOTALES</td><td class="n">${NOC.App.money(t.base)}</td><td class="n">${NOC.App.money(t.iva)}</td><td class="n">${NOC.App.money(t.re)}</td><td class="n">${NOC.App.money(t.total)}</td></tr></tfoot>
      </table>
      <div class="count">${currentRows.length} documento(s)</div>
      <script>window.onload=()=>{window.print();}</script>
      </body></html>`);
    w.document.close();
  }

  return{render,generar,exportCsv,imprimir};
})();