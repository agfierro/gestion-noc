...
function taxFor(c){
 ...
 async function facturar(id){
  ...
  try{
    const data=await NOC.API.rpc("facturar_proforma_atomica_v406",{p_proforma_id:id,p_fecha_factura:NOC.App.today()});
    NOC.App.hideProgress();
 ...
 async function facturarSeleccionadas(){
  ...
  for(let i=0;i<seleccionadas.length;i++){
    ...
    try{
      const data=await NOC.API.rpc("facturar_proforma_atomica_v406",{p_proforma_id:p.id,p_fecha_factura:NOC.App.today()});
      const numero=Array.isArray(data)?data[0]?.numero_factura:data?.numero_factura;
